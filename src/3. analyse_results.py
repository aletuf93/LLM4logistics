...
fig, axs = plt.subplots(numLLMs, 4, figsize=(20, 20))
...
for i_column, model in enumerate(dict_column_models.keys()):
    ...
    # set the current axis
    plt.sca(axs[numLLMs-1, i_column])
    ...
    createHeatmap(df_analysis,
                ...
                listMethods=listMethods)
    # set x-label
    axs[numLLMs-1, i_column].set_xlabel("Method")
    # set y-label
    if i_column==0:
        axs[numLLMs-1, i_column].set_ylabel("Problem")
